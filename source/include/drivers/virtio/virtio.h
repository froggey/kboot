...
static inline struct vring_desc *virtio_desc_index_to_desc(struct virtio_device *dev, unsigned int ring_index, uint16_t desc_index)
{
    assert(desc_index != 0xffff);
...
void virtio_kick(struct virtio_device *dev, unsigned int ring_idnex);

void virtio_irq_wait(struct virtio_device *dev);

#ifdef CONFIG_DRIVER_VIRTIO_BLOCK
extern status_t virtio_block_init(struct virtio_device *dev, uint32_t host_features);
#endif

#define LTRACEF(x...) do { if (LOCAL_TRACE) { dprintf(x); } } while (0)
