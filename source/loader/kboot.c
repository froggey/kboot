...
kboot_vaddr_t kboot_alloc_virtual(kboot_loader_t *loader, kboot_paddr_t phys, kboot_vaddr_t size, int cache_mode) {
    load_ptr_t addr;
    ...
    if (phys != ~(kboot_paddr_t)0) {
        ...
        if (!mmu_map(loader->mmu, addr, phys, size, cache_mode, true))
            boot_error("Invalid virtual mapping (physical 0x%" PRIx64 ")", phys);
    }
 ...
 * @param phys          Physical address to map to (or ~0 for no mapping).
 * @param size          Size of the range. */
void kboot_map_virtual(kboot_loader_t *loader, kboot_vaddr_t addr, kboot_paddr_t phys, kboot_vaddr_t size, int cache_mode) {
...
static void setup_trampoline(kboot_loader_t *loader) {
    ...
    loader->trampoline_virt = kboot_alloc_virtual(loader, loader->trampoline_phys, PAGE_SIZE, MMU_CACHE_NORMAL);
    ...
    mmu_map(loader->trampoline_mmu, loader_start, loader_phys, loader_size, MMU_CACHE_NORMAL, true);
    mmu_map(loader->trampoline_mmu, loader->trampoline_virt, loader->trampoline_phys, PAGE_SIZE, MMU_CACHE_NORMAL, true);
...
static void set_video_mode(kboot_loader_t *loader) {
    ...
    case VIDEO_MODE_VGA:
        ...
        tag->vga.mem_virt = kboot_alloc_virtual(loader, mode->mem_phys, mode->mem_size, MMU_CACHE_WRITETHROUGH);
        break;
    case VIDEO_MODE_LFB:
        /* TODO: Indexed modes. */
        ...
        tag->lfb.fb_virt = kboot_alloc_virtual(loader, mode->mem_phys, mode->mem_size, MMU_CACHE_WRITETHROUGH);
        break;
    }
}
...
static __noreturn void kboot_loader_load(void *_loader) {
    ...
    kboot_itag_foreach(loader, KBOOT_ITAG_MAPPING, kboot_itag_mapping_t, mapping) {
        if (mapping->virt == ~(kboot_vaddr_t)0) {
            kboot_alloc_virtual(loader, mapping->phys, mapping->size, MMU_CACHE_UNCACHED);
        } else {
            kboot_map_virtual(loader, mapping->virt, mapping->phys, mapping->size, MMU_CACHE_UNCACHED);
        }
    }
    ...
    loader->tags_virt = kboot_alloc_virtual(loader, loader->core->tags_phys, KBOOT_TAGS_SIZE, MMU_CACHE_NORMAL);
    ...
    loader->core->stack_base = kboot_alloc_virtual(loader, phys, PAGE_SIZE, MMU_CACHE_NORMAL);
